	...
	def connection (self):
        ...
        def list (self):
            ...
            if self._search_index:
                    sql = "SELECT type AS type, path AS url "
                    sql+= "FROM searchIndex GROUP BY type "
            else:
                    sql = "SELECT count(*), ztypename AS type "
                    sql+= "FROM ztoken JOIN ztokenmetainformation "
                    sql+= "ON ztokenmetainformation.z_pk = ztoken.zmetainformation "
                    sql+= "JOIN ztokentype ON ztokentype.z_pk = ztoken.ztokentype GROUP BY type"
...
if __name__ == '__main__':
	docsets = os.path.expanduser("~/.local/share/Zeal/Zeal/docsets/")
